...
def test_produccion_no_usa_mocks():
    fabrica = PasarelaProduccion()
    assert not isinstance(fabrica.crear_pago_tarjeta("0000"), PagoMock)
    assert not isinstance(fabrica.crear_pago_efectivo(), PagoMock)


def test_pruebas_no_usa_reales():
    fabrica = PasarelaPruebas()
    assert not isinstance(fabrica.crear_pago_tarjeta("0000"), PagoTarjeta)
    assert not isinstance(fabrica.crear_pago_efectivo(), PagoEfectivo)
